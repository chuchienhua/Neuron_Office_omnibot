...
from std_msgs.msg import Bool as JoyBool
...
class Omni_base_node:
	def __init__(self):
		...
		self.param["vel_cmd"] = rospy.get_param("~omni_base_driver/cmd_vel", 'omni_base_driver/cmd_vel')
		...
		self.param["imuId"] = rospy.get_param('~imu_id', 'base_imu') # odom frame id
		...
		self.joy_sub = rospy.Subscriber("joy_enable", JoyBool, self.joy_control, queue_size=10)
	...
	*******************************************************************'''
	...
	'''*******************************************************************
		ROS cmd_vel Subscriber callback
	*******************************************************************'''
	def joy_control(self, joy_enable):
		rospy.loginfo("joy_enable.data: %s", joy_enable.data)
		self.joy_control_enable = joy_enable.data
		rospy.loginfo("joy_control_enable: %s", self.joy_control_enable)
